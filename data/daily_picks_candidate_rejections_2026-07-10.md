# Daily Picks Candidate Rejection Report

Monitoring-only diagnostic artifact. Not official picks. Not buy instructions.

- Date: **2026-07-10**
- Primary no-pick cause: **NO_PICK_ALL_FINALISTS_HARD_BLOCKED**
- Summary: **No official picks were generated because all 2 finalist candidate(s) were hard-blocked.**
- Paper trading enabled: **false**
- Live trading enabled: **false**

## Pre-Hard-Block Finalists
- PERF: score=1.0, sector=N/A, trade_type=day, entry=1.74, stop_loss=1.72, take_profit=1.77, risk_reward=1.5, news_action_window=intraday, watch_only=False
- DAL: score=0.912, sector=N/A, trade_type=day, entry=89.0, stop_loss=87.23, take_profit=91.94, risk_reward=1.66, news_action_window=intraday, watch_only=False

## Hard-Blocked Finalists
- PERF: **penny_stock** — penny stock ($1.74 < $5.0) (trade_type=day, watch_only=False)
- DAL: **sl_too_tight** — SL too tight (2.0% < 2.0% for $89 stock) (trade_type=day, watch_only=False)
