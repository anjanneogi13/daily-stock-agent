...
for p in month_picks:
    ...
    if p.get("evaluation_status") in ("tp_hit", "sl_hit", "expired"):
        ...
        if p["evaluation_status"] == "tp_hit": weekly[w]["tp"] += 1
        if p["evaluation_status"] == "sl_hit": weekly[w]["sl"] += 1
...
for w in sorted(weekly.keys()):
    ...
    weekly_summary.append({
        "week_starting": w, "picks": d["picks"], "evaluated": d["evaluated"],
        "tp": d["tp"], "sl": d["sl"], "win_rate_pct": round(wr, 1),
        "avg_r": round(avg_r, 3), "avg_return_pct": round(avg_ret, 2),
...
for i in range(1, len(weekly_summary)):
    ...
    trend.append({
        "from_week": prev["week_starting"], "to_week": curr["week_starting"],
...
def git_changes():
    try:
        ...
        cmd = ["git", "log", f"--since={month_start}", "--pretty=format:%ad|%s", "--date=short", "--"] + files
...
stats = {"by_score": stat_bucket(score_bucket),
...
prompt = f"""You are a senior quantitative analyst writing a MONTHLY RETROSPECTIVE for an
