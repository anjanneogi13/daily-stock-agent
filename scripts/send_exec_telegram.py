...
lines = [f"📊 *Execution Report — {date}*\n",
         f"✅ TP: {len(tp)} | 🛑 SL: {len(sl)} | ⏳ Open: {len(op)} | ⚠️ Unfilled: {len(nf)}\n"]

for r in picks:
    t = r["ticker"]; s = r["status"]
    if s == "NO_DATA":
        lines.append(f"*{t}* — _no data_"); continue
    if s == "NOT_FILLED":
        lines.append(f"⚠️ *{t}* — limit not hit (missed by {r['missed_by_pct']:+.2f}%)"); continue
    if s == "TP_HIT":
        lines.append(
            f"✅ *{t}* TP +{r['tp_pct']:.2f}% • left {r['further_after_tp_pct']:+.2f}% on table\n"
            f"   _no-TP close: {r['no_tp_close_pct']:+.2f}%_")
    elif s == "SL_HIT":
        lines.append(
            f"🛑 *{t}* SL {r['sl_pct']:.2f}% • dropped {r['further_after_sl_pct']:+.2f}% more\n"
            f"   _no-SL worst: {r['no_sl_worst_pct']:.2f}%, close: {r['no_sl_close_pct']:+.2f}%_")
    else:  # OPEN
        lines.append(
            f"⏳ *{t}* close {r['close_ret_pct']:+.2f}% • MFE {r['mfe_pct']:+.2f}% MAE {r['mae_pct']:+.2f}%")
