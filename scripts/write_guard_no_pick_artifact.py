...
- NO_PICK_AGENT_PAUSED
- NO_PICK_DUPLICATE_ALREADY_LOGGED
...
# Causes that must never be written on a day that already has official
# (non watch-only) picks logged. Defense-in-depth against a stray or manual
# invocation rewriting a real pick day as a no-pick day.
CAUSES_BLOCKED_WHEN_OFFICIAL_PICKS_EXIST = {
    "NO_PICK_WINDOW_MISSED",
    "NO_PICK_AGENT_PAUSED",
    "NO_PICK_DUPLICATE_ALREADY_LOGGED",
}
...
def _status_for_cause(cause: str) -> tuple[str, str, str]:
    ...
    if cause == "NO_PICK_AGENT_PAUSED":
        return "not_ready_agent_paused", "healthy", "agent_paused"
    if cause == "NO_PICK_DUPLICATE_ALREADY_LOGGED":
        return "not_ready_duplicate_run", "healthy", "duplicate_same_day_run"
...
def _default_reason(cause: str, date_str: str) -> str:
    ...
    if cause == "NO_PICK_AGENT_PAUSED":
        return (
            "No official premarket pick was generated because the agent is auto-paused "
            "by its safety system. Paused runs must not generate official picks; the "
            "pause is a deliberate protective decision, not a failure."
        )

    if cause == "NO_PICK_DUPLICATE_ALREADY_LOGGED":
        return (
            "No official premarket pick was generated by this run because a same-day "
            "run already logged today's outcome. Duplicate multi-fire runs must not "
            "produce a second official decision."
        )
...
def main() -> int:
    ...
    # Never claim "no pick" on a day that already has official picks logged.
    # The workflow guard also dedups first, but this keeps a stray/manual
    # invocation from overwriting a real pick day.
    if args.cause in CAUSES_BLOCKED_WHEN_OFFICIAL_PICKS_EXIST:
        already = official_picks_logged_for_date(args.date)
        if already > 0:
            print(
                f"⏭ Skipping {args.cause} artifact: {already} official pick(s) "
