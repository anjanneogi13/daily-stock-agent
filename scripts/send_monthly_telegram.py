...
_REPORT_TYPE = "monthly_xray"
_TODAY = _dt.now().strftime("%Y-%m-%d")
if not should_send_report(_REPORT_TYPE, _TODAY):
    print(f"[telegram] ⏭  Monthly X-Ray for {_TODAY} already sent — skipping (set FORCE_RESEND=1 to override)")
    sys.exit(0)
print(f"[telegram] ✓ Monthly X-Ray for {_TODAY} — sending...")
